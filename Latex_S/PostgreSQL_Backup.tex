...
\item PostgreSQL备份的几种方式
    \begin{enumerate}
        \item SQL dump\\
        SQL 转储的方法采用的方式是创建一个文本文件，这个文本里面都是 SQL 命令，把这个文件回馈给slave服务器，”重播“这些SQL操作后，将重建与转储时状态一样的数据库.
...
这种热备方式设定主从服务器，所有的修改查询发向master服务器，数据改变后以异步的方式将改变的数据发向standby服务器让其更新同步，
而在均衡负载方面，standby服务器可以分担一些数据库的查询操作请求。
...
基于中间件这种方式，程序将会拦截每一个SQL请求操作，如果是查询读操作就将它发送到其中任意一个服务器，
如果是修改写操作，则将它发送到所有的服务器让每个服务器各自执行SQL修改写入操作。
...
对于这种方案可以通过设置一写多读或者用共享硬盘减少通信来达到提高性能的目的。
